...
const run_package_script_1 = __importDefault(require("../run-package-script"));
...
function handler(argv) {
    return __awaiter(this, void 0, void 0, function* () {
        (0, sitecore_jss_dev_tools_1.verifySetup)();
        const packageJson = yield (0, resolve_package_1.default)();
        if (!packageJson.config.sitecoreDistPath) {
            console.error(chalk_1.default.red('The current project does not support file deployment into the Sitecore instance. You should use an HTTP POST based integration for Experience Editor support. See SDK documentation for details.'));
            return;
        }
        if (!argv.skipBuild && argv.buildTaskName && argv.buildTaskName.length > 0) {
            (0, run_package_script_1.default)([argv.buildTaskName]);
        }
        const options = {
            ...
            if (!options.sourcePath) {
                ...
                if (!options.sourcePath) {
                    ...
                    if (fs_1.default.existsSync('build/index.html')) {
                        ...
                        console.error(chalk_1.default.red('No source path specified, conventional build paths missing, and buildArtifactsPath config is not defined in package.json!'));
                        return;
                    }
                }
            }
            if (!options.destinationPath) {
                const scJssConfig = yield (0, sitecore_jss_dev_tools_1.resolveScJssConfig)({ configPath: argv.config });
                options.destinationPath = path_1.default.join(scJssConfig.sitecore.instancePath, packageJson.config.sitecoreDistPath);
            }
        }
        (0, sitecore_jss_dev_tools_1.deploy)(options);
    });
}
