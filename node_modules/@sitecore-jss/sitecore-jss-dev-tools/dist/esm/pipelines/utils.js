...
export const resolveFiles = ({ fileGlob, extensions, recursive, workingDirectory, allowEmptyGlobs = false, }) => {
    let files = [];
    ...
    readdirSync(lookupPath).forEach((file) => {
        ...
        try {
            ...
            if (stat.isDirectory()) {
                if (recursive) {
                    files = files.concat(resolveFiles({ fileGlob: filePath, extensions, recursive, workingDirectory }));
                }
                return;
            }
        }
...
export const importModules = ({ fileGlobs = [], workingDirectory = '', allowEmptyGlobs = false, }) => {
    const resolved = fileGlobs.reduce((initialResult, fileGlob) => {
        let result = initialResult;
        if (fileGlob) {
            result = result.concat(resolveFiles({
                fileGlob,
                extensions: ['.js'],
                recursive: true,
                workingDirectory,
