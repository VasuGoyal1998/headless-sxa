...
function setup(interactive, outputFile, initialData, configName = 'sitecore') {
    ...
    if (interactive) {
        configObject.deployUrl = getInteractiveData(configObject.deployUrl || (0, url_join_1.default)(configObject.layoutServiceHost, defaultDeployUrl), interactive, 'host', 'Sitecore import service URL', '(usually same as hostname)', getValidation(/^https?:\/\/(.*)/), 'Invalid input. Must start with http(s)');
    }
    // API KEY
    ...
    if (configObject.deploySecret) {
        ...
        if (appConfig) {
            const appName = (0, find_app_name_1.findAppNameInConfig)(appConfig);
            if (appName) {
                const patchFile = path_1.default.resolve(`./sitecore/config/${appName}.deploysecret.config`);
                (0, secret_patch_1.writeSecretPatchFile)(patchFile, appName, configObject.deploySecret);
                ...
                console.log((0, secret_patch_1.createSecretPatchContents)('YOUR-JSS-APP-NAME-HERE', configObject.deploySecret));
                console.log('');
            }
        }
        else {
