...
const generateFields = (component, rendering, dataSourceItem, allComponents) => {
    ...
    if (component) {
        ...
        renderingFields = (0, utils_1.validateFieldDefinitions)(rendering.fields, component, handleError, allComponents);
    }
    return (0, utils_1.convertComponentDataToFields)({ data: renderingFields, context: { item: dataSourceItem } });
};
const generateChildrenFields = (children) => {
    const [...result] = children;
    result.forEach((child) => {
        if (!(0, manifest_types_1.isItemDefinition)(child)) {
            return;
        }
        if (child.fields) {
            child.fields = (0, utils_1.convertComponentDataToFields)({
                data: child.fields,
                context: { item: child },
            });
        }
...
const generateRenderingUid = (renderingName, renderingIndex, parentPlaceholderKey) => {
    ...
    const parentNamespace = (0, uuid_1.v5)(renderingName + parentPlaceholderKey, JSS_UUID_NAMESPACE);
    // 2. Using the parent UUID, we create a deterministic UUID for the rendering using its index in the placeholder
    const renderingUid = (0, uuid_1.v5)(renderingIndex.toString(), parentNamespace);
...
const processRendering = (rendering, index, context) => {
    ...
    const component = (0, utils_1.findComponent)(rendering.componentName, context.components);
