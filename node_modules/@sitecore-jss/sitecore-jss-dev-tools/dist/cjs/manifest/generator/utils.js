...
const filterObject = (obj, filter) => Object.keys(obj).reduce((res, key) => {
    if (filter(key, obj[key])) {
        return Object.assign(Object.assign({}, res), { [key]: obj[key] });
    }
    return res;
}, {});
exports.filterObject = filterObject;
const convertComponentDataToFields = ({ data, context: { item = {} } = {}, }) => {
    if (!data) {
        return;
    }
...
exports.convertComponentDataToFields = convertComponentDataToFields;
const mapFieldValueItem = (item, fieldValueItem, index, fieldName) => {
    ...
    fieldItem.fields = (0, exports.convertComponentDataToFields)({
        data: fieldValueItem.fields,
        context: { item: fieldItem },
    });
    return fieldItem;
};
const processFieldValue = ({ fieldValue, context: { fieldName, item } }) => {
    if (fieldValue === null) {
        return fieldValue;
    }
    ...
    if (Array.isArray(fieldValue)) {
        ...
        return fieldValueItems.map((fieldValueItem, index) => mapFieldValueItem(item, fieldValueItem, index, fieldName));
    }
 ...
 * @returns {ComponentDefinition | undefined} component
 */
function findComponentForTemplate(templateName, components) {
    var _a;
    if (!components) {
        return undefined;
    }
 ...
 */
function findTemplate(templateName, templates) {
    if (!templates) {
        return undefined;
    }
 ...
 * @returns {TemplateDefinition | undefined} template
 */
function findTemplateForComponent(component, templates) {
...
function validateFieldDefinitions(fields, template, handleError, ...inheritedTemplates) {
    ...
    return (0, exports.filterObject)(fields, (fieldName) => {
