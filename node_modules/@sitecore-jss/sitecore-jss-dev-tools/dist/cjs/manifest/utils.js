...
const resolveFiles = ({ fileGlob, extensions, recursive, workingDirectory, }) => {
    ...
    if (!(0, fs_extra_1.existsSync)(lookupPath)) {
        if ((0, fs_extra_1.existsSync)(`${lookupPath}.js`)) {
            lookupPath += '.js';
        }
        else {
            const options = {};
    ...
    try {
        const stat = (0, fs_extra_1.statSync)(lookupPath);
        if (stat.isFile()) {
    ...
    (0, fs_extra_1.readdirSync)(lookupPath).forEach((file) => {
        const filePath = nodePath.join(lookupPath, file);
        let stat;
        try {
            stat = (0, fs_extra_1.statSync)(filePath);
            if (stat.isDirectory()) {
                if (recursive) {
...
const importModules = ({ fileGlobs = [], workingDirectory = '', }) => {
    ...
    fileGlobs.forEach((fileGlob) => {
        resolved = resolved.concat((0, exports.resolveFiles)({ fileGlob, extensions: ['.js'], recursive: true, workingDirectory }));
    });
    if (!resolved.length) {
    ...
    const modules = resolved.map((file) => () => (0, import_fresh_1.default)(nodePath.resolve(workingDirectory, file)));
    return modules;
};
